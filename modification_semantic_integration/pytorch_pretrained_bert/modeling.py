...
class BertPreTrainedModel(nn.Module):
    ...
    @classmethod
    def from_cached(cls, pretrained_model_name_or_path, state_dict=None, cache_dir=None,
                        from_tf=False, *inputs, **kwargs):
        """
        Instantiate a BertPreTrainedModel from a pre-trained model file or a pytorch state dict.
        Download and cache the pre-trained model file if needed.
...
#         resolved_archive_file = os.path.join(cache_dir, archive_file)
...
#                 archive.extractall(tempdir)
#             serialization_dir = tempdir
        print("loading model from cache dir {0}".format(cache_dir))
        serialization_dir = cache_dir
        # Load config
        config_file = os.path.join(serialization_dir, CONFIG_NAME)
        config = BertConfig.from_json_file(config_file)
        logger.info("Model config {}".format(config))
...
class SemanticIntegrationMLP1(nn.Module):
    def __init__(self, config, tag_config):
        super(SemanticIntegrationMLP1, self).__init__()
...
class SemanticIntegrationMLP2(nn.Module):
    def __init__(self, config, tag_config):
        ...
        self.cnn_hidden_size = config.hidden_size
        self.tag_hidden_size = tag_config.hidden_size
        self.layer1_tag = nn.Linear(self.tag_hidden_size, self.cnn_hidden_size * 2)
