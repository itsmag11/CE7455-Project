...
class BertForSequenceClassificationTag(BertPreTrainedModel):
    def __init__(self, config, num_labels=2, tag_config=None):
        ...
        use_tag = True
        if use_tag:
            # print(f'pool input dim: {config.hidden_size} + {tag_config.hidden_size}')
            # print(f'pool output dim: {config.hidden_size} + {tag_config.hidden_size}')
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_end_idx=None, input_tag_ids=None, labels=None):
        # print(f'{input_ids.size()} | {token_type_ids.size()} | {attention_mask.size()}')
        # torch.Size([32, 128]) | torch.Size([32, 128]) | torch.Size([32, 128])
        sequence_output, _ = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)
        # print(f'{sequence_output.size()}')
        # torch.Size([32, 128, 768])

        no_cuda = False
        batch_size, sub_seq_len, dim = sequence_output.size()
        # sequence_output = sequence_output.unsqueeze(1)
        start_end_idx = start_end_idx  # batch * seq_len * (start, end) torch.Size([32, 128, 2])
        max_seq_len = -1
        max_word_len = self.filter_size
        # print(f'original max word len: {max_word_len}')

        ## split BERT sequence output to sub_words
        for se_idx in start_end_idx: ## seq_len * (start, end)
            # print(f'{se_idx.size()}')
            num_words = 0
            for item in se_idx: ## (start, end)
                if item[0] != -1 and item[1] != -1: ## if the word is not a single sub-word word
                    # print('exist')
        ...
        # print(f'new max word len: {max_word_len}') ## max number of sub-words in a word, 4
        # print(f'new max seq len: {max_seq_len}') ## max number of words in a sequence, 91
        assert max_word_len >= self.filter_size

        batch_start_end_ids = []
        batch_id = 0
        for batch in start_end_idx: ## seq_len * (start, end)
            word_seqs = []
            ...
            for item in batch: ## (start, end)
                if item[0] != -1 and item[1] != -1:
                    ...
                    while len(subword_ids) < max_word_len:
                        subword_ids.append(0) ## pad 0 to words shorter than max (all pad to max length)
                    # [3979, 0, 0]
                    # [3980, 0, 0]
                    # [3981, 3982, 3983]
                    word_seqs.append(subword_ids) ## represent word sequence by equal length sub-words
                    ## [[3979, 0, 0], [3980, 0, 0], [3981, 3982, 3983]]
            # print(f'before: {word_seqs}, {len(word_seqs)}') ## one batch corresponds to one word sequence
            while (len(word_seqs) < max_seq_len):
                word_seqs.append([0 for i in range(max_word_len)])
                ## pad [0, 0, 0, 0] to word sequences shorter than max
            # print(f'after: {word_seqs}, {len(word_seqs)}')
            batch_start_end_ids.append(word_seqs)
            batch_id += 1

        batch_start_end_ids = torch.tensor(batch_start_end_ids)
        # print(f'{batch_start_end_ids.size()}') # torch.Size([32, 91, 4])
        ## each batch has 91 words and each word with 4 sub-words

        batch_start_end_ids = batch_start_end_ids.view(-1)
        # print(f'{batch_start_end_ids.size()}') #torch.Size([11648]) 11648=32x91x4
        sequence_output = sequence_output.view(-1, dim)
        # print(f'{sequence_output.size()}') #torch.Size([4096, 768]) 4096=32x128
        sequence_output = torch.cat([sequence_output.new_zeros((1, dim)), sequence_output], dim=0)
        # print(f'{sequence_output.size()}') #torch.Size([4097, 768]) pad one row of all zeros in dim=0
        if not no_cuda:
            batch_start_end_ids = batch_start_end_ids.cuda()
            # print(f'{batch_start_end_ids.size()}') #torch.Size([11648])
        cnn_bert = sequence_output.index_select(0, batch_start_end_ids)
        # print(f'{cnn_bert.size()}') #torch.Size([11648, 768])
        cnn_bert = cnn_bert.view(batch_size, max_seq_len, max_word_len, dim)
        # print(f'{cnn_bert.size()}') #torch.Size([32, 91, 4, 768])
        if not no_cuda:
            cnn_bert = cnn_bert.cuda()

        bert_output = self.cnn(cnn_bert, max_word_len)
        # print(f'{bert_output.size()}') #torch.Size([32, 91, 768])

        use_tag = True
        if use_tag: #SRL tag
            ## input_tag_ids: torch.Size([32, 3, 128])
            num_aspect = input_tag_ids.size(1)
            # print(f'{num_aspect}') #3
            input_tag_ids = input_tag_ids[:,:,:max_seq_len]
            # print(f'{input_tag_ids.size()}') #torch.Size([32, 3, 91])
            flat_input_tag_ids = input_tag_ids.view(-1, input_tag_ids.size(-1))
            # print(f'{flat_input_tag_ids.size()}') #torch.Size([96, 91])
            # print("flat_que_tag", flat_input_que_tag_ids.size())
            tag_output = self.tag_model(flat_input_tag_ids, num_aspect)
            # print(f'{tag_output.size()}') #torch.Size([32, 3, 91, 10])
            # batch_size, que_len, num_aspect*tag_hidden_size
            tag_output = tag_output.transpose(1, 2).contiguous().view(batch_size,
                                                                      max_seq_len, -1)
            # print(f'{tag_output.size()}') #torch.Size([32, 91, 30])
            tag_output = self.dense(tag_output)
            # print(f'{tag_output.size()}') #torch.Size([32, 91, 10])
            sequence_output = torch.cat((bert_output, tag_output), 2)
            # print(f'{sequence_output.size()}') #torch.Size([32, 91, 778])
            # print("tag", tag_output.size()) #tag torch.Size([32, 91, 10])
            # print("bert", bert_output.size()) #bert torch.Size([32, 91, 768])
        ...
        first_token_tensor, pool_index = torch.max(sequence_output, dim=1)
        # print(f'first_token_tensor: {first_token_tensor.size()}') #torch.Size([32, 91])
        # print(f'pool_index: {pool_index.size()}') #torch.Size([32, 91])
