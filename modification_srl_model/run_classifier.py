    ...
    def _read_txt(cls, input_file):
        ...
        with open(input_file, "r", encoding='utf-8') as f:
            ...
            for idx, line in enumerate(reader):
                # title line is removed in example preparation later
                # if idx == 0:
                #     continue
...
class SnliSelfProcessor(DataProcessor):
  """Processor for the SNLI data set (GLUE version)."""

  def get_train_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_txt(os.path.join(data_dir, "train.txt_tag")), "train")

  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_txt(os.path.join(data_dir, "dev.txt_tag")), "dev")

  def get_test_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_txt(os.path.join(data_dir, "test.txt_tag")), "test")
...
def transform_tag_features(max_num_aspect, features, tag_tokenizer, max_seq_length):
    ...
    for example in features:
        ...
        # print(f'tag_ids_list_a: {tag_ids_list_a}')
        input_tag_ids = []
...
def main():
    ...
    parser.add_argument("--feature_name",
                        type=str, default='snli',
                        help="name for saved train feature (if exist, load directly)")
    ...
    result_dir = os.path.join('./results', args.output_dir)
    ...
    # logger file
    global logger
    logger = create_logger(output_dir=args.output_dir, name=f"{'log'}")
    logger.info(f'====================================================')
    logger.info(args)
    logger.info(f'====================================================')
    ...
    train_feature_path = os.path.join('./saved_train_features/', '{}.pkl'.format(args.feature_name))
    if os.path.exists(train_feature_path):
        print('loading saved train features ...', flush=True)
        with open(train_feature_path, 'rb') as f:
            train_features = pickle.load(f)
    else:
        print('no saved train features, preparing now ...', flush=True)
        train_features = None
        if args.do_train:
    ...
    if args.do_train:
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(tqdm(train_dataloader)):
                ...
                # print(f'{input_ids.size()} | {input_mask.size()} | {segment_ids.size()} | {start_end_idx.size()} | {input_tag_ids.size()} | {label_ids.size()}')
                # torch.Size([32, 128]) | torch.Size([32, 128]) | torch.Size([32, 128]) | torch.Size([32, 128, 2]) | torch.Size([32, 3, 128]) | torch.Size([32])
                ...
                nb_tr_steps += 1
                # logger.info(f'[{step}/{len(train_dataloader)}] loss: {tr_loss/nb_tr_steps}')
                summary_writer.add_scalar('train/train_loss_step', loss.item(), global_step)
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    optimizer.zero_grad()
                    global_step += 1
            logger.info(f'[{epoch}] loss: {tr_loss / nb_tr_steps}')
            summary_writer.add_scalar('train/train_loss_epoch', tr_loss/nb_tr_steps, epoch)
            ...
            if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                ...
                # logger.info(f'[Validation] loss: {eval_loss} | acc: {eval_accuracy}')
                summary_writer.add_scalar('val/val_loss_epoch', eval_loss, epoch)
                summary_writer.add_scalar('val/val_acc_epoch', eval_accuracy, epoch)
